...
namespace ts {
    ...
    export function createTypeChecker(host: TypeCheckerHost, produceDiagnostics: boolean): TypeChecker {
        ...
        function resolveName(location: Node, name: string, meaning: SymbolFlags, nameNotFoundMessage: DiagnosticMessage, nameArg: string | Identifier): Symbol {
            ...
            {
                var sourceFile = getSourceFileOfNode(lastLocation);
                var dependFile = getSourceFileOfNode(result.declarations[0]);
        ...
        function checkTypeReferenceNode(node: TypeReferenceNode | ExpressionWithTypeArguments) {
            ...
            if (type && type.symbol&&type.symbol.declarations)
            {
                ...
                var dependFile = getSourceFileOfNode(type.symbol.declarations[0]);
                if (sourceFile && dependFile)
                {
                    ts.UsedFileResolver.mapFile(sourceFile.fileName, dependFile.fileName);
                }
